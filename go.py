...
class SphereServer(object):
    def __init__(self, sphere_model, sphere_simulator, args):
        self.args = args
    ...
    def _create_services(self):
        ...
        # Show runner
        self.runner = ShowRunner(self.sphere_model, self.sphere_simulator,
    ...
    def start(self):
        if self.running:
            print "start() called, but sphere is already running!"
            return
        ...
        try:
            ...
            print "Exception starting Spheres!"
            traceback.print_exc()
    ...
    def stop(self):
        if self.running: # should be safe to call multiple times
            try:
                ...
                print "Exception stopping Spheres!"
                traceback.print_exc()
    ...
    def go_web(self, app):
        ...
        cherrypy.quickstart(SphereWeb(app),
                '/',
                config=config)

class SphereWeb(object):
...
if __name__=='__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Spheres Light Control')
    ...
    print "Using Sphere Simulator at %s:%d" % (sim_host, sim_port)

    from model.simulator import SimulatorModel
    model = SimulatorModel(sim_host, port=sim_port)
    full_spheres = sphere.load_spheres(model)

    app = SphereServer(full_spheres, model, args)
    try:
        ...
        print "Unhandled exception running Spheres!"
        traceback.print_exc()
    finally:
        app.stop()
